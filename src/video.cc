...
Video::Video() {
   ...
   // setup window
   this->window = SDL_CreateWindow("glsl_sandbox",
...
void Video::setViewport() {
   int width  = SDL_GetWindowSurface(this->window)->w;
   int height = SDL_GetWindowSurface(this->window)->h;
...
int Video::height() const {
   return SDL_GetWindowSurface(this->window)->h;
}

int Video::width() const {
   return SDL_GetWindowSurface(this->window)->w;
}
